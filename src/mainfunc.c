...
bool addRule(sqlite3 *db, const Rule *rule) 
{
    ...
    sprintf(sql, "INSERT INTO rules (protocol, src_ip, dst_ip, src_port, dst_port, start_time, end_time, action, remarks) "
                ...
                rule->protocol, rule->src_ip, rule->dst_ip, rule->src_port, rule->dst_port, rule->start_time, 
...
void importRules(const char *filename, sqlite3 *db) 
{
    ...
    while (fgets(line, sizeof(line), file)) 
    {
        ...
        char endDate[11];
        char endTime[9];
        sscanf(line, "%d %s %s %s %s %s %10s %8s %10s %8s %d %s", &rule->id, rule->protocol, rule->src_ip,
...
void exportRules(const char *filename, sqlite3 *db) 
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) 
    {
        ...
        rule.src_ip, rule.dst_ip, (strcmp(rule.src_port, "") != 0) ? rule.src_port : "NULL",
        (strcmp(rule.dst_port, "") != 0) ? rule.dst_port : "NULL", rule.start_time,
        rule.end_time, rule.action, (strcmp(rule.remarks, "") != 0) ? rule.remarks : "NULL");
...
void printRules(sqlite3* db) 
{
    ...
    printf("\033[1;93m| ID | Protocol |     Src IP    |     Dst IP    | Src Port | Dst Port |     Start Time       |      End Time        | Action |     Remarks      |\033[0m\n");
    printf("\033[1;93m+----+----------+---------------+---------------+----------+----------+----------------------+----------------------+--------+------------------+\033[0m\n");
    ...
    while (sqlite3_step(statement) == SQLITE_ROW) 
    {
        ...
        const char* src_port = (const char*)sqlite3_column_text(statement, 4);
        const char* dst_port = (const char*)sqlite3_column_text(statement, 5);
        ...
        printf("| %-2d | %-8s | %-13s | %-13s | %-8s | %-8s | %-20s | %-20s | %-6d | %-16s |\n",
               id, protocol, src_ip, dst_ip, src_port, dst_port, start_time, end_time, action, remarks);
    }
...
bool writeRulesToDevice(sqlite3* db) 
{
    ...
    while (sqlite3_step(statement) == SQLITE_ROW) 
    {
        ...
        if(action == 1){continue;}
         ...
         // 端口为空时写入0
        src_port = (strcmp(src_port, "") != 0) ? src_port : "0";
        dst_port = (strcmp(dst_port, "") != 0) ? dst_port : "0";
